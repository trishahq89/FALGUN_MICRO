...
public class firstTestNG_printUserName {
	...
	//we used getAttribute() for printing the links as well	
		
//>>>>>>>>>>>>>>>>>>>>>>//PRINT username and password
		
		//which link are we looking for? need the location 
		//driver.findElement(By.id("loginusername")).getAttribute("value"); //returning string value, hold it in a string
		
		
		String user = driver.findElement(By.id("loginusername")).getAttribute("value");
		//what kind of value it is returning me? string or integer? its string, so hold it in a variable
...
//>>>>>>>>>>>>>>>>>>>>>>>>> print password	
		...
		String pass = driver.findElement(By.id("loginpassword")).getAttribute("value");
...
//>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>PRINT PROFILEPAGE		
		...
		String profileName = driver.findElement(By.xpath("//a[text()='Welcome trishanewyork']")).getAttribute("value");
		System.out.println(profileName);
//NULL: WE DID NOT ENTER ANYTHING THAT WHY ITS NULL, USER ID AND PASSWORD WE ENTERED SO THE VALUES ARE SHOWN	
	//THIS IS NOT APPLICABLE FOR GETVALUE, SINCE WE DID NOT ENTER ANYTHING	
